...
def extract_mesh(occ_hat, threshold=0.2, padding=0.1, n_face_simp=None):
...
def compute_iou(occ1, occ2, iou_thres=0.5):
    ''' Computes the Intersection over Union (IoU) value for two sets of
    occupancy values.
...
def compute_chamfer_distance_p2p(points_src, points_tgt):
    ''' Computes chamfer distances of each point in points_src to points_tgt.
